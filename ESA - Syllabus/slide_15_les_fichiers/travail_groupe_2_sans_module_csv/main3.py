...
for config_hote in liste_hotes:
    ...
    dico_complet = dico_ssh_config_template | config_hote
    configurations_hotes.append(dico_complet)
...
def obtenir_noms_hotes(hotes):
    ...
    """
    # Extraire la valeur de la clé 'Host' pour chaque hôte dans la liste.
    return [hote.get('Host', '') for hote in hotes]
...
def compter_utilisateur(hotes, utilisateur):
    ...
    """
    # Parcourir chaque hôte et compter ceux dont la clé 'User' a la valeur spécifiée.
    return sum(1 for hote in hotes if hote.get('User', '') == utilisateur)


# 5. Affichage des résultats
print(f"Nombre d'hôtes : {compter_hotes(configurations_hotes)}")
print(f"Noms des hôtes : {', '.join(obtenir_noms_hotes(configurations_hotes))}")
print(f"Nombre de user \"etd-esa\" : {compter_utilisateur(configurations_hotes, 'etd-esa')}")


"""
    L'opérateur "|" n'est disponible qu'à partir de Python 3.9 !
    
        merged_config = dico_ssh_config_template | config_hote 
    
     
    Pour les anciennes versions de Python :  

        merged_config = dico_ssh_config_template.copy()
        merged_config.update({k: v for k, v in config_hote.items() if v})
"""